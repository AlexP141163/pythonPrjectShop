...
class CartAddView(CartMixin, View):
    def post(self, request):
        ...
        cart = self.get_cart(request, product=product)   # Подмешивается из 'Mixin':

        if cart:    # Если пользователь авторизован:
            cart.quantity += 1
            cart.save()
        else:       # Если пользователь не авторизован, корзина будет создаваться через 'session_key':
        ...
        response_data = {
            ...
            'cart_items_html': self.render_cart(request)    # Подмешивается из 'Mixin':
        }
